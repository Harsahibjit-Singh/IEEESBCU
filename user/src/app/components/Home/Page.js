...
const defaultData = {
  'branch name': { content: 'IEEE Chandigarh University Student Branch' },
  'branch short name': { content: 'IEEE CUSB' },
  'rank in india council': { content: '1' },
  'main carousel': {
    images: [
  ...
  'active members': { content: '500+' },
  'events organized': { content: '50+' },
  'branch age': { content: '10+ Years of Excellence' },
  'our mission': { content: 'To foster technological innovation and excellence for the benefit of humanity...' },
  'our vision': { content: 'To be essential to the global technical community...' },
  'branch logo': { images: ['/image.png'] }
};
...
export default function HomePage() {
 ...
 // Theme styles
  ...
  // Fetch all branch data from Supabase
  useEffect(() => {
  ...
  // Carousel effect
  useEffect(() => {
    if (!isHovering && branchData['main carousel']?.images?.length > 0) {
      const interval = setInterval(() => {
        setCurrent((prev) => (prev + 1) % branchData['main carousel'].images.length);
      }, 5000);
      return () => clearInterval(interval);
    }
  }, [isHovering, branchData]);

  const nextSlide = () => {
    if (branchData['main carousel']?.images?.length > 0) {
      setCurrent((prev) => (prev + 1) % branchData['main carousel'].images.length);
    }
  };
  ...
  const prevSlide = () => {
    if (branchData['main carousel']?.images?.length > 0) {
      setCurrent((prev) => (prev - 1 + branchData['main carousel'].images.length) % branchData['main carousel'].images.length);
    }
  };
  ...
  if (isLoading) {
    return (
      <div className={`min-h-screen w-full ${pageBg} flex items-center justify-center`}>
  ...
  return (
    <div className={`min-h-screen w-full ${pageBg}`}>
      <div className={`w-full max-w-7xl mx-auto px-4 py-8 ${style.text}`}>
        {/* Hero Carousel */}
        {branchData['main carousel']?.images?.length > 0 && (
          <div
          ...
          >
            <AnimatePresence mode='wait'>
              <motion.img
                key={current}
                src={branchData['main carousel'].images[current]}
                alt={`Slide ${current + 1}`}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
            ...
            <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-3 z-20">
              {branchData['main carousel'].images.map((_, idx) => (
                <button
                  key={idx}
        ...
        >
          {[
            { number: branchData['rank in india council']?.content || '1st', title: "Place in IEEE India Council" },
            { number: branchData['active members']?.content || '500+', title: "Active Members" },
            { number: branchData['events organized']?.content || '50+', title: "Events Organized" },
            { number: branchData['branch age']?.content || '10+', title: "Years of Excellence" }
          ].map((item, index) => (
            <div
              key={index}
          ...
          <div className="p-8 md:p-12 flex flex-col md:flex-row items-center gap-8 md:gap-12">
            ...
            <div className="flex-1">
              ...
              <div className="space-y-4">
                <p className="text-lg">
                  {branchData['about above']?.content || 'Established in 2014, our Student Branch has consistently pushed boundaries in technological innovation and professional development.'}
                </p>
                <p className={`text-lg font-medium p-4 rounded-xl border-l-4 ${theme === 'light' ? 'bg-blue-100 border-blue-300' : 'bg-white/10 border-blue-300'}`}>
                  Our relentless efforts earned us the <span className={theme === 'light' ? 'font-bold text-blue-800' : 'font-bold text-blue-100'}>
                    {branchData['rank in india council']?.content || 'First'} Place at the IEEE India Council
                  </span>, making us one of the most active branches worldwide.
                </p>
                <p className="text-lg">
                  {branchData['about below']?.content || 'We regularly organize technical sessions, expert talks, hackathons, and workshops to prepare students for the challenges of tomorrow\'s technological landscape.'}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 mt-16">
          {[
            { 
              type: 'Mission',
              content: branchData['our mission']?.content || 'To foster technological innovation and excellence for the benefit of humanity...'
            },
            { 
              type: 'Vision',
              content: branchData['our vision']?.content || 'To be essential to the global technical community...'
            }
          ].map((item, index) => (
            <motion.div
              key={index}
            ...
            >
              <div className="flex items-center mb-6">
                ...
                <h3 className="text-2xl font-bold">{`Our ${item.type}`}</h3>
              </div>
              <p className="text-lg">
                {item.content}
              </p>
